...
void ting_main_Init(void)
{
	...
	//printf_P("xzw=\n");
	//printf_P("xzw=%d\n",50);
	persistence_init();//EEPROM
	stepper_init();// 步进子系统
	encoder_init();	// 虚拟编码器
					//switch_init();	// 开关
					//pwm_init();			// 脉宽调制驱动程序
	controller_init(0, 0, 0);// 控制器初始化
	config_init();					// 从eeprom配置记录 - 必须是下一个app init
	network_init();					// 如果需要，重置标准设备 - 必须遵循config_init（）
	planner_init();					// 运动规划子系统
	canonical_machine_init();		// 规范机器 - 必须遵循config_init（）
	rpt_print_system_ready_message();//（LAST）宣布系统准备就绪	
}
void ting_Loop(void)
{
	controller_run();
