...
stat_t cm_arc_feed(const float target[], const bool target_f[],     // 目标端点
                   const float offset[], const bool offset_f[],     // IJK offsets
                   const float radius, const bool radius_f,         // 半径如果是半径模式
                   const float P_word, const bool P_word_f,         // 参数
                   ...
                   const uint8_t motion_mode)                       // defined motion mode
{
    ...
    // 测试是否在所选平面中指定了端点.test if no endpoints are specified in the selected plane
    arc.full_circle = false;        //初始条件 initial condition
    if (!(target_f[arc.plane_axis_0] || target_f[arc.plane_axis_1])) {
        if (radius_f) {             // 在半径模式中，缺少两个端点的弧是错误的
            return (STAT_ARC_AXIS_MISSING_FOR_SELECTED_PLANE);
        } else {
            arc.full_circle = true; // 以中心格式弧，这指定一个完整的圆
        }
    }
